...
import {
  createTask, getPackageManager, getTaskName, isAutoDetectionEnabled,
  isWorkspaceFolder, NpmTaskDefinition,
  NpmTaskProvider,
  startDebugging,
  TaskWithLocation
} from './tasks'
...
export class NpmScriptsTreeDataProvider implements TreeDataProvider<TreeItem> {
  ...
  constructor (private readonly context: ExtensionContext,
    ...
    subscriptions.push(commands.registerCommand('xpm.runScript',
      this.runScript, this))
    subscriptions.push(commands.registerCommand('xpm.debugScript',
      this.debugScript, this))
    subscriptions.push(commands.registerCommand('xpm.openScript',
      this.openScript, this))
    subscriptions.push(commands.registerCommand('xpm.runInstall',
  ...
  private async runInstall (selection: PackageJSON): Promise<void> {
    ...
    const task = await createTask(await getPackageManager(this.context,
      selection.folder.workspaceFolder.uri, true), 'install', ['install'],
    selection.folder.workspaceFolder, uri, undefined, [])
  ...
  private buildTaskTree (tasks: TaskWithLocation[]): TaskTree {
    ...
    tasks.forEach(each => {
      if (isWorkspaceFolder(each.task.scope) &&
        ...
        const definition: NpmTaskDefinition =
          each.task.definition as NpmTaskDefinition
